...
def profile_single_file(sc, file):
    ...
    for i in range(len(header)):
        ...
        column_information.append([{"column_name": str(header[i]), "number_non_empty_cells":int(number_non_empty), "number_empty_cells": int(
            number_empty), "number_distinct_values":int(number_distinct), "frequent_values": top_five_freq}])

    # extra credit
    most_unique_column = max(np.array(num_unique_value))
    index_key_column = num_unique_value.index(most_unique_column)

    basic_information.append([{"file_name": str(
        sys.argv[1]), "column": column_information, "key_column": str(header[index_key_column])}])
    ...
    for i in range(len(header)):
        find_data_type = lines_without_header.map(lambda x: (
            data_type(x[i]), 1)).reduceByKey(lambda x, y:	x+y).collect()
